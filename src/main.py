...
if __name__ == '__main__':
    ...
    registered_models = source._get_mlflow_registered_models(max_results=1)
    for rm in registered_models:
        print(f"--- {rm.name} ---")
        model_versions = source._get_mlflow_model_versions(registered_model=rm, max_results=1)
